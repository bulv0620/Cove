...
图床状态加载时使用主内容区居中的加载占位。SMB 未启用、当前用户未绑定、绑定配置失效或状态请求失败时，页面与 Files 一样以顶部栏以下的整个主内容区显示居中不可用占位，保留原因、帮助文案、刷新和有权限用户的管理入口，不显示图床标题、说明、工具栏或图片内容卡片。SMB 可用后才显示完整图床页头与管理界面，内容在主区域内滚动。
...
- 图床删除先撤销公开 grant，再删除已登记的 SMB 原图对象和由同一指纹生成的缩略图；成功 API 返回 `204 No Content`。原图或缩略图已不存在视为幂等清理成功。其他失败进入 `CLEANUP_PENDING`；后台恢复只在路径和 SMB 对象身份都匹配时重试，不按文件名前缀批量删除。
